...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const toggleDark = () => {
    ...
    if (next) {
      document.documentElement.classList.add("dark");
      localStorage.setItem("cren_theme", "dark");
    } else {
      document.documentElement.classList.remove("dark");
      localStorage.setItem("cren_theme", "light");
    }
  };
        ...
        <div className="flex items-center justify-between px-6 py-2">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="w-8 h-8 flex items-center justify-center rounded-full bg-white/15 hover:bg-white/30 border border-white/20 transition-all cursor-pointer shrink-0"
            >
              {isDark ? (
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-yellow-300" viewBox="0 0 24 24" fill="currentColor">
